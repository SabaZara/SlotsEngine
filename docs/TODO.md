...
| `game-backend/src/routes/*.ts` | ~250 | **`rounds.ts` done** — 27 tests, all nine mutations caught, covering the typed error mappings (404/400/402 each distinct) that e2e cannot practically enumerate. Requests are signed properly rather than bypassing service-auth. Remaining: `bonus`, `simulate`, `public`, `launchTokens`, `serviceAuth` — all still exercised by the e2e suites, which is real coverage, but a failure there names a flow rather than a branch. |
